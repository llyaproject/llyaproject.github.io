...
void main () {
  ...
  color.r *= noise(vPos * 0.01 - uTime);
  color.g *= noise(vPos * 0.02 - uTime);
  color.b *= noise(vPos * 0.08 - uTime);
...
class Mouse {
  ...
  onTouchmove(e) {
    ...
    this.speed =
      Math.sqrt(
        (touch.pageX - this.lastX) ** 2 + (touch.pageY - this.lastY) ** 2
      ) * 0.5;
...
class Sketch {
  ...
  updateCamera(time) {
    this.cameraV
      .subVectors(this.mouse.mouse, this.cameraP)
      .multiplyScalar(0.05);
    this.cameraP.add(this.cameraV);

    this.camera.position.set(
      this.cameraP.x * this.dist * 0,
      this.cameraP.y * this.dist * 0,
      this.dist * 0
    );
  ...
  updateLight() {
    this.spotLightV
      .subVectors(this.mouse.mouse, this.spotLightP)
      .multiplyScalar(0.05);
...
class Shape {
  ...
  initialize() {
    ...
    // sphere
    this.sphereGeometry = new THREE.TorusGeometry(
      this.sphereSize - this.sphereSize / 2,
      this.sphereSize / 2 / 2,
      128,
      128
    );
    ...
    this.sphereMeshP = new THREE.Vector3(0, this.sphereSize / 2, 0);
  ...
  render(time) {
    ...
    this.sphereMesh.rotation.x = -time * this.sketch.mouse.mouse.y;
  ...
  updateData(t) {
    for (var y = 0; y < this.height; y++) {
      for (var x = 0; x < this.width; x++) {
        ...
        this.d.data[i + 0] = 0xff;
        this.d.data[i + 1] = 0xff;
        this.d.data[i + 2] = 0xff;
        this.d.data[i + 3] = x & y & (x ^ y) % Math.floor(t % 10) ? 0xff : 0x02;
      }
    }
  }
